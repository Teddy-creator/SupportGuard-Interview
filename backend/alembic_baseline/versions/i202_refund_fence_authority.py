"""Use the active run/job fence as the refund proposal authority.

Revision ID: i202_refund_fence_authority
Revises: i201_retire_escalation

The ticket status is a converging customer projection.  Proposal authorization
already requires the exact active AgentRun, RuntimeJob fencing token, reserved
Policy capability, causal-decision hash, observation-binding hash, scoped
billing record, duplicate relation, and current policy evidence.  Keeping an
additional refund-only ticket-status gate made this one action sensitive to a
projection race that the other two actions correctly ignore.
"""
...
revision: str = "i202_refund_fence_authority"
down_revision: str | None = "i201_retire_escalation"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None


_AUTHORIZE_REFUND_BY_FENCE_SQL = r"""
  ...
  IF p_capability_name='propose_refund' THEN
    IF (SELECT count(*) FROM jsonb_object_keys(p_model_arguments))<>3
       OR NOT p_model_arguments ?& ARRAY[
         'billing_record_id','refund_reason','idempotency_key'] THEN
...
def upgrade() -> None:
    ...
    op.execute(_AUTHORIZE_REFUND_BY_FENCE_SQL)


def downgrade() -> None:
    raise RuntimeError("interview_refund_fence_authority_downgrade_forbidden")
