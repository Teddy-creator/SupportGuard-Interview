...
copied from the current evidence: chunk_id, document_id, version, content_hash, and locator_hash.
The deterministic Runtime resolves that immutable `source-locator.v2` hash against the pinned
snapshot; never invent, stringify, or repair citation metadata.
